...
class PendingMemberEmailHandler():
    ...
    def __init__(self, recipient_list, pending_user_list, mail_credentials):
        """
    ...
    def _get_message_subject_line(num_days):
        """
        Figure out what severity term we want to use for the outgoing message (based on how long clients have been waiting in queue)
...
""" % (len(self.pending_user_list), num_days, '\n'.join([p[1] + ' (' + p[2] + ')' for p in self.pending_user_list]), sys.argv[0])
    ...
    def _create_message_object(self, num_days):
        ...
        mime_message['to'] = self.recipient_list  # TODO: Check that giving this a list of string emails actually works
